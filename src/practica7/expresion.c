...
double evaluar(Arbol arbol)
{
	...
	// AQUÍ
	double resultado = evaluarNodo(arbol.raiz);
	printf("\nResultado: %lf\n", resultado);
...
double evaluarNodo(NodoA *raiz)
{
    ...
    {
        return **(var->v); // valor capturado
    }

    // OPERADOR
    double izq = evaluarNodo(raiz->izq);
    double der = evaluarNodo(raiz->dch);
