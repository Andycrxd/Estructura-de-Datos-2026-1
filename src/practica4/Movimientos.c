...
Movimientos movimientosPosibles(Laberinto lab,Coordenada punto)
{
	...
	if( lab[punto.x][punto.y+1] == '*' || lab[punto.x][punto.y+1] == 'B')
		movimientos.right = 1;
	//IZQUIERDA
	if( lab[punto.x][punto.y-1] == '*' || lab[punto.x-1][punto.y-1] == 'B' )
		movimientos.left = 1;
	return movimientos;	
}

Movimientos movimientosPosiblesRegreso(Laberinto lab,Coordenada punto)
{
	Movimientos movimientos = {0,0,0,0};
	
	
	//ARRIBA
	if( lab[punto.x-1][punto.y] == '.' || lab[punto.x-1][punto.y] == 'A')
		movimientos.up = 1;
	//ABAJO
	if( lab[punto.x+1][punto.y] == '.'|| lab[punto.x+1][punto.y] == 'A')
		movimientos.down = 1;
	//DERECHA
	if( lab[punto.x][punto.y+1] == '.' || lab[punto.x][punto.y+1] == 'A')
		movimientos.right = 1;
	//IZQUIERDA
	if( lab[punto.x][punto.y-1] == '.' || lab[punto.x][punto.y-1] == 'A')
		movimientos.left = 1;
